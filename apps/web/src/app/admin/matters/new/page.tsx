    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: '400px', padding: '2rem' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 900, color: 'var(--ink)', marginBottom: '0.75rem' }}>
            Matter Created
          </div>
          <p style={{ fontSize: '0.88rem', color: 'var(--ink-muted)', lineHeight: 1.7, marginBottom: '2rem' }}>
            The client has been notified by email. Go to the pipeline to send the payment link.
          </p>
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button
              onClick={() => window.location.href = '/admin'}
              style={{ background: 'var(--ink)', color: 'var(--white)', padding: '0.75rem 1.5rem', borderRadius: '4px', border: 'none', fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}
            >
              Go to Pipeline
            </button>
            <button
              onClick={() => { setSubmitted(false); setForm({ clientId: '', title: '', service: '', description: '', fee: '', slaHours: 48, dueDate: '', region: 'INDIA', internalNotes: '' }); }}
              style={{ background: 'transparent', color: 'var(--ink)', padding: '0.75rem 1.5rem', borderRadius: '4px', border: '1.5px solid var(--border-strong)', fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer' }}
            >
              Create Another
            </button>
          </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, height: '60px', padding: '0 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--ink)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <button
          onClick={() => window.location.href = '/admin'}
          style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.5)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)' }}
        >
          ← Back to Dashboard
        </button>
      </nav>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '3rem 2rem' }}>
        <div style={{ marginBottom: '2.5rem' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '0.5rem' }}>
            Admin · Matter Creation
          </div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', fontWeight: 900, color: 'var(--ink)', letterSpacing: '-0.02em' }}>
            Create New Matter
          </h1>
          <p style={{ fontSize: '0.85rem', color: 'var(--ink-muted)', marginTop: '0.4rem' }}>
            After filling this form the client receives an email notification and payment link is ready to trigger from the pipeline.
          </p>
        </div>

        {error && (
          <div style={{ background: '#FEE2E2', border: '1px solid #FCA5A5', borderRadius: '6px', padding: '0.85rem 1rem', marginBottom: '1.5rem', fontSize: '0.85rem', color: '#991B1B' }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '10px', padding: '1.5rem' }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-soft)', marginBottom: '1rem' }}>
                Client
              </div>

              {selectedClient ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.85rem 1rem', background: 'var(--gold-pale)', border: '1.5px solid var(--gold)', borderRadius: '6px' }}>
                  <div>
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-muted)', fontSize: '0.8rem', fontFamily: 'var(--font-body)' }}
                  >
                    Change
                  </button>
                </div>
                ...
                <>
                  ...
                  <div style={{ maxHeight: '200px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    {filteredClients.length === 0 ? (
                      <div style={{ padding: '1rem', textAlign: 'center', fontSize: '0.82rem', color: 'var(--ink-faint)' }}>
                      ...
                      filteredClients.map((c) => (
                        <button
                          ...
                          style={{ padding: '0.75rem 1rem', background: 'var(--paper)', border: '1px solid var(--border)', borderRadius: '6px', cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--font-body)', transition: 'all 0.15s' }}
            ...
            <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-soft)' }}>
                Matter Details
              </div>
              ...
              <div>
                ...
                <select value={form.service} onChange={(e) => handleChange('service', e.target.value)} style={inputStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                {form.fee && (
                  <div style={{ fontSize: '0.72rem', color: 'var(--ink-muted)', marginTop: '0.3rem' }}>
                    + 18% GST = {form.region === 'INDIA' ? '₹' : '$'}{Math.round(parseInt(form.fee) * 1.18).toLocaleString('en-IN')} total
                  </div>
                )}
              </div>
            ...
            {/* Internal notes */}
            <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '10px', padding: '1.5rem' }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-soft)', marginBottom: '0.75rem' }}>
                Internal Notes
              </div>
              <textarea
                ...
                style={{ ...inputStyle, resize: 'vertical' }}
              />
              <div style={{ fontSize: '0.7rem', color: 'var(--ink-faint)', marginTop: '0.4rem' }}>
            ...
            {form.clientId && form.title && form.fee && (
              <div style={{ background: 'var(--ink)', borderRadius: '10px', padding: '1.5rem' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '1rem' }}>
                  Matter Summary
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.25rem' }}>
                  {[
                    { label: 'Client', value: selectedClient ? `${selectedClient.firstName} ${selectedClient.lastName}` : '' },
                    { label: 'Service', value: form.service },
                    { label: 'Fee', value: `${form.region === 'INDIA' ? '₹' : '$'}${parseInt(form.fee).toLocaleString('en-IN')} + GST` },
                ...
                <button
                  ...
                  style={{
                    ...
                    color: 'var(--ink)', border: 'none', borderRadius: '4px',
                ...
                >
                  {loading ? 'Creating Matter...' : 'Create Matter & Notify Client →'}
                </button>
              </div>
            )}
          </div>
